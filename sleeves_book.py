#!/usr/bin/env python3
"""
SLEEVES — the eight-sleeve book, in one file.

Every sleeve below is the VERIFIED version: parameters from the sweeps, exits
from the exit audit, and the concurrency cap that the forensic audit added.
Pulled from hourly_sim.py so the live bot has a single reference.

THE BOOK
  delta     60 coins, taker-flow z over 7d, hold 14, N=5      1.41
  relvol    24 coins, volume vs 20d baseline, N=8, hold 7     1.17
  skew      24 coins, 60d realised skew, n=6, hold 45         1.26
  cascade   market z <= -2.5, long 5 worst, hold 3d           0.49
  sr        6h resistance break +OI, 3ATR stop, hold 15       2.02
  fvg       12h fair value gap +OI, 1ATR stop, hold 10        3.49
  pattern   6h MarkitTick bullish +OI, 2ATR stop, hold 15     1.81
  srflip    6h resistance-turned-support retest, hold 15      1.71

  BOOK (equal-risk, netted)  3.93 Sharpe, bull 3.59, bear 4.26, maxDD 4.6%

CRITICAL — the concurrency bug. A fixed per-position size does NOT cap how many
positions open at once. FVG ran at a mean of 12.5 concurrent when its sizing
assumed 6, i.e. 2.1x levered. Capping it RAISED Sharpe 3.29 -> 3.49 and cut
max drawdown 91% -> 28%. Every event sleeve passes max_gross=1.0.

CRITICAL — entry timing. Event sleeves enter at the open of the bar AFTER the
signal, never at the historical break bar. That error once inflated a backtest
from 1.04 to 6.06.

Data required:
  taker_data/<COIN>_1h.csv   open_time,open,high,low,close,volume,taker_buy,delta
                             MATCHED VENUE (Binance futures). clean_panel's
                             taker column is another venue's volume -- taker
                             exceeded TOTAL volume on 60% of bars.
  oi_data/<COIN>_oi_1h.csv   timestamp,open_interest
"""
