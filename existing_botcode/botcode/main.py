...
class HyroTraderBot:
    ...
    def _execute_entry(self, signal: Signal):
        ...
        # --- BTC MACD REGIME SIZING (zero-line, 250 candles) ---
        try:
            _btc = self.client.get_klines("BTCUSDT", "60", 250)
            if _btc and len(_btc) >= 40:
                _closes = [float(_k['close']) for _k in _btc]
                def _ema_series(_vals, _p):
                    _k2 = 2.0 / (_p + 1.0)
                    _e = None
                    _out = []
                    for _v in _vals:
                        _e = _v if _e is None else _v * _k2 + _e * (1.0 - _k2)
                        _out.append(_e)
                    return _out
                _e12 = _ema_series(_closes, 12)
                _e26 = _ema_series(_closes, 26)
                _macd = [_a - _b for _a, _b in zip(_e12, _e26)]
                _macd_val = _macd[-2]
                _btc_dir = 1 if _macd_val > 0 else -1
                _sig_dir = 1 if signal.side == "BUY" else -1
                if _btc_dir == -_sig_dir:
                    _old_risk = signal.risk_usd
                    signal.risk_usd = signal.risk_usd * 0.25
                    signal.qty = signal.qty * 0.25
                    logger.info(f"BTC-REGIME {signal.symbol}: {signal.side} FIGHTS BTC MACD (macd={_macd_val:+.1f}, bars={len(_closes)}) risk ${_old_risk:.0f}->${signal.risk_usd:.0f}")
                else:
                    logger.info(f"BTC-REGIME {signal.symbol}: {signal.side} ALIGNED with BTC MACD (macd={_macd_val:+.1f}) full size ${signal.risk_usd:.0f}")
            else:
                logger.warning(f"BTC-REGIME skipped {signal.symbol}: insufficient BTC data - full size")
        except Exception as _btc_err:
            logger.warning(f"BTC-REGIME check failed ({_btc_err}) - full size")
        # --- END BTC MACD REGIME SIZING ---
        # Risk exposure check
        ...
        # ─── WORST-CASE SIZING CAP ───
        SAFETY_FLOOR = ACCOUNT.initial_balance * 0.9025   # $180,500 (was 0.92/184k - lowered so bot sizes down instead of freezing)
        ...
        for _p in self.positions.values():
            if not _p.tp1_hit:
                existing_wc_loss += _p.risk_usd
        room = (balance - existing_wc_loss) - SAFETY_FLOOR
        if room <= 0:
            logger.warning(f"⚠️ SKIP {signal.symbol}: worst-case sizing — no room "
                           f"(bal=${balance:,.0f} - open_wc=${existing_wc_loss:,.0f} < ${SAFETY_FLOOR:,.0f})")
            return
        if signal.risk_usd > room:
            _sl_dist = abs(signal.entry_price - signal.sl_price)
            if _sl_dist <= 0:
                logger.warning(f"⚠️ SKIP {signal.symbol}: invalid sl_dist")
                return
